...
**Note for Monorepo Development**: This plugin is part of a monorepo and depends on `@ideatr/core`. If you're working within the monorepo structure:
...
**Important**: The plugin uses `@ideatr/core` via `"@ideatr/core": "file:../core"` in `package.json`. The core package must be built before the plugin can be built. See [docs/core/deployment/DEPLOYMENT_ARCHITECTURE.md](../docs/core/deployment/DEPLOYMENT_ARCHITECTURE.md) for details.
