...
describe('FileManager', () => {
    ...
    describe('createIdeaFile', () => {
        ...
        it('should return created file', async () => {
            ...
            expect(file.path).toBe('Ideas/Test idea.md');
        });
        ...
        it('should handle filename collisions with suffix', async () => {
            ...
            vi.mocked(vault.getAbstractFileByPath)
                ...
                .mockReturnValueOnce({ path: 'Ideas/Test idea.md' } as any) // First collision check
                .mockReturnValueOnce(null); // Second collision check (no collision)

            await fileManager.createIdeaFile(idea);

            expect(vault.create).toHaveBeenCalledWith(
                'Ideas/Test idea-2.md',
                expect.any(String)
            );
        });
        ...
        it('should handle multiple collisions', async () => {
            ...
            vi.mocked(vault.getAbstractFileByPath)
                .mockReturnValueOnce(null) // Directory check
                .mockReturnValueOnce({ path: 'Ideas/Test idea.md' } as any)
                .mockReturnValueOnce({ path: 'Ideas/Test idea-2.md' } as any)
                .mockReturnValueOnce({ path: 'Ideas/Test idea-3.md' } as any)
                .mockReturnValueOnce(null);

            await fileManager.createIdeaFile(idea);

            expect(vault.create).toHaveBeenCalledWith(
                'Ideas/Test idea-4.md',
                expect.any(String)
            );
        });
    });
